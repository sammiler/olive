...
bool CurveView::FirstChanceMousePress(QMouseEvent *event) {
  ...
  if (dragging_bezier_pt_) {
    ...
    dragging_bezier_point_start_ =
        (dragging_bezier_pt_->type == NodeKeyframe::kInHandle) ? key->bezier_control_in() : key->bezier_control_out();
    dragging_bezier_point_opposing_start_ =
        (dragging_bezier_pt_->type == NodeKeyframe::kInHandle) ? key->bezier_control_out() : key->bezier_control_in();
...
void CurveView::FirstChanceMouseRelease(QMouseEvent *event) {
  ...
  if (!(event->modifiers() & Qt::ControlModifier)) {
    ...
    command->add_child(new KeyframeSetBezierControlPoint(dragging_bezier_pt_->keyframe, opposing_type,
...
void CurveView::KeyframeDragMove(QMouseEvent *event, QString &tip) {
  ...
  for (size_t i = 0; i < GetSelectedKeyframes().size(); i++) {
    ...
    key->set_value(FloatSlider::TransformDisplayToValue(
        FloatSlider::TransformValueToDisplay(drag_keyframe_values_.at(i).toDouble(), display) - scaled_diff, display));
  }
...
void CurveView::ZoomToFitSelected() { ZoomToFitInternal(true); }
