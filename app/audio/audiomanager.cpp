...
void AudioManager::SetOutputNotifyInterval(int n) { output_buffer_->set_notify_interval(n); }
...
bool AudioManager::PushToOutput(const AudioParams &params, const QByteArray &samples, QString *error) {
  ...
  if (output_params_ != params || output_stream_ == nullptr) {
    ...
    PaError r = Pa_OpenStream(&output_stream_, nullptr, &p, output_params_.sample_rate(), paFramesPerBufferUnspecified,
                              paNoFlag, OutputCallback, output_buffer_);
    if (r != paNoError) {
      // Unhandled error
