...
void MathNodeBase::ValueInternal(Operation operation, Pairing pairing, const QString &param_a_in,
  ...
  switch (pairing) {
    case kPairNumberNumber: {
      if (val_a.type() == NodeValue::kRational && val_b.type() == NodeValue::kRational && operation != kOpPower) {
        ...
        output->Push(NodeValue::kRational,
                     QVariant::fromValue(
                         PerformAddSubMultDiv<rational, rational>(operation, val_a.toRational(), val_b.toRational())),
                     this);
      } else {
    ...
    case kPairMatrixVec: {
      ...
      PushVector(output, qMax(val_a.type(), val_b.type()), PerformMult<QVector4D, QMatrix4x4>(operation, vec, matrix));
      break;
    }
    ...
    case kPairSampleSample: {
      ...
      if (max_samples > min_samples) {
        ...
        for (int i = 0; i < mixed_samples.audio_params().channel_count(); i++) {
          memcpy(&mixed_samples.data(i)[min_samples], &larger_buffer.data(i)[min_samples], remainder * sizeof(float));
        }
      }
    ...
    case kPairTextureMatrix: {
      ...
      job.SetShaderID(QStringLiteral("%1.%2.%3.%4")
                          .arg(QString::number(operation), QString::number(pairing), QString::number(val_a.type()),
...
MathNodeBase::Pairing MathNodeBase::PairingCalculator::GetMostLikelyPairing() const { return most_likely_pairing_; }

const NodeValue &MathNodeBase::PairingCalculator::GetMostLikelyValueA() const { return most_likely_value_a_; }

const NodeValue &MathNodeBase::PairingCalculator::GetMostLikelyValueB() const { return most_likely_value_b_; }
