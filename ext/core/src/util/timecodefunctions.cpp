...
std::string Timecode::time_to_timecode(const rational &time, const rational &timebase, const Timecode::Display &display,
  ...
  switch (display) {
    ...
    case kTimecodeSeconds: {
      ...
      if (display == kTimecodeSeconds) {
        ...
        return StringUtils::format("%s%s:%s:%s.%s", prefix, StringUtils::to_string_leftpad(hours, 2).c_str(),
        ...
        return StringUtils::format("%s%s:%s:%s%s%s", prefix, StringUtils::to_string_leftpad(hours, 2).c_str(),
                                   StringUtils::to_string_leftpad(mins, 2).c_str(),
...
rational Timecode::timecode_to_time(std::string timecode, const rational &timebase, const Timecode::Display &display,
                                    bool *ok) {
...
std::string Timecode::time_to_string(int64_t ms) {
  ...
  return StringUtils::format("%s:%s:%s", StringUtils::to_string_leftpad(hh, 2).c_str(),
...
bool Timecode::timebase_is_drop_frame(const rational &timebase) { return (timebase.numerator() != 1); }
